...
for ($i=0; $i<count($arr);$i++) {
  ...
  if ($percent == -1) {
    ...
    <ul id='context-".$arr[$i][0]."' class='dropdown-content'>
        <li><a class=\"modal-trigger\" onclick=\"event.stopPropagation(); projid = ". $arr[$i][0] ."; $('#editProjText').val('". $arr[$i][1] ."'); $('#modal3').modal('open')\"><!--<i class='material-icons'>edit</i>-->Edit</a></li>
        <li><a class=\"modal-trigger\" onclick=\"event.stopPropagation(); projid = '". $arr[$i][0] ."'; vis = '".$arr[$i][2]."'; chipifyVis(); $('#visName').text('". $arr[$i][1] ."'); $('#modal4').modal('open')\"><!--<i class='material-icons'>visibility</i>-->Visibility</a></li>
        <li class='divider'></li>
        <li><a class=\"modal-trigger\" onclick=\"event.stopPropagation(); projid = ". $arr[$i][0] ."; $('#delProjName').text('". $arr[$i][1] ."'); $('#modal2').modal('open')\"><!--<i class='material-icons'>delete</i>-->Delete</a></li>
...
echo "
			...
			";
	if (in_array('admin', $_SESSION['priv'])) {
		echo('<a class="waves-effect waves-light btn modal-trigger projectAdd" href="#modal1">Add</a>');
	}
	echo "
