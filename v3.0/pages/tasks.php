...
function tasksPage() {
  ...
  if (count($arr) > 0) {
   	echo "<ul class='collapsible'>";
        ...
        if (in_array('admin', $_SESSION['priv'])) {
          $match = true;
        } else {
          $match = false;
      ...
      <ul id='context-".$arr[$i][0]."' class='dropdown-content'>
        <li><a class=\"modal-trigger\" onclick=\"event.stopPropagation(); taskid = '". $arr[$i][0] ."'; $('#editTaskText').val('". $arr[$i][1] ."'); $('#modal6').modal('open')\"><!--<i class='material-icons'>edit</i>-->Edit</a></li>
        <li><a class=\"modal-trigger\" onclick=\"event.stopPropagation(); taskid = '". $arr[$i][0] ."'; vis = '".$arr[$i][2]."'; chipifyTVis(); $('#visTaskName').text('". $arr[$i][1] ."'); $('#modal7').modal('open')\"><!--<i class='material-icons'>visibility</i>-->Visibility</a></li>
        <li class='divider'></li>
        <li><a class=\"modal-trigger\" onclick=\"event.stopPropagation(); taskid = '". $arr[$i][0] ."'; $('#delTaskName').text('". $arr[$i][1] ."'); $('#modal5').modal('open')\"><!--<i class='material-icons'>delete</i>-->Delete</a></li>
      </ul>
        ...
        if (count($sarr) > 0) {
          echo "<table><thead><tr><th>Step</th><th>Complete</th></tr></thead><tbody>";
              ...
              <ul id='context-".$sarr[$j][0]."' class='dropdown-content'>
        			<li><a class=\"modal-trigger\" href=\"#modal4\" onclick=\"event.stopPropagation(); stepid = '". $sarr[$j][0] ."'; $('#editStepText').val('".$sarr[$j][1]."'); $('#modal4').modal('open')\"><!--<i class='material-icons'>edit</i>-->Edit</a></li>
              <li><a class=\"modal-trigger\" href=\"#modal10\" onclick=\"event.stopPropagation(); stepid = '". $sarr[$j][0] ."'; $('#editStepAssigned').val('".$sarr[$j][4]."'); $('#modal10').modal('open')\">Assign</a></li>
        			<li><a class=\"modal-trigger\" href=\"#modal8\" onclick=\"event.stopPropagation(); stepid = '". $sarr[$j][0] ."'; vis = '".$sarr[$j][3]."'; chipifySVis(); $('#visStepName').text('". $sarr[$j][1] ."'); $('#modal8').modal('open')\"><!--<i class='material-icons'>visibility</i>-->Visibility</a></li>
              <li class='divider'></li>
        			<li><a class=\"modal-trigger\" href=\"#modal3\" onclick=\"event.stopPropagation(); stepid = '". $sarr[$j][0] ."'; $('#delStepName').text('". $sarr[$j][1] ."'); $('#delStepTaskName').text('". $arr[$i][1] ."'); $('#modal3').modal('open')\"><!--<i class='material-icons'>delete</i>-->Delete</a></li>
     				 	</ul></td></tr>"
  ...
  <a class="waves-effect waves-light btn modal-trigger stepAdd" href="#modal1" onclick="projid = \''. $_SESSION['projid'] .'\'; taskid = \''. $arr[$i][0] .'\'">Add</a>
    ...
    echo "</ul>";
    	...
    	echo "<h4>There are no tasks. Add some below</h4><style>.content {display: flex; justify-content: center; align-items: center;}</style>";
			...
			echo "<h4>There are no tasks</h4><style>.content {display: flex; justify-content: center; align-items: center;}</style>";
		}
	}
	if ($_SESSION['projedit']) {
  	echo "
