...
function getProjects() {
  $conn = connect();
  $qr = $conn->query('SELECT * FROM `projects` WHERE `visibility` REGEXP \','.implode(',|,', $_SESSION['priv']).',\' AND `bid`='.$_SESSION['bid']);
...
function rnewuser($user, $hash, $type, $add, $add2) {
	$conn = connect();
	$priv = 'all,'.$user.',edit-'.$user;
	$bid = 0;
	if ($type) {
		$qr = $conn->query("INSERT INTO `companies` (`name`, `code`, `admin`) VALUES ('$add', '$add2', '$user')");
		$bid = $conn->insert_id;
	} else {
		$qr = $conn->query("SELECT * FROM `companies` WHERE code='$add'");
		while ($row = mysqli_fetch_array($qr, MYSQLI_ASSOC)) {
			$abid[] = $row["bid"];
		}
		$bid = $abid[0];
	}
	$conn->query("INSERT INTO `logins` (`username`, `password`, `priviliges`, `bid`) VALUES ('$user', '$hash', '$priv', '$bid')");
	$conn->close();
	return true;
}
...
function checkCode($code) {
	$conn = connect();
	$qr = $conn->query("SELECT * FROM `companies` WHERE code='$code'");
	$conn->close();
...
function createCode() {
	$rand = rand(pow(10, 4), pow(10, 5)-1);
	while (checkCode($rand) != "invalid") {
		$rand = rand(pow(10, 4), pow(10, 5)-1);
	}
	return $rand;
}
function checkName($projName) {
	$conn = connect();
	$qr = $conn->query("SELECT * FROM `companies` WHERE name='$projName'");
	$conn->close();
	if (mysqli_num_rows($qr)==0) {
		return "false";
	} else {
	...
	if (mysqli_num_rows($qr)==0) {
      ...
      $bid[] = $row['bid'];
		...
		if (!password_verify($hash, $pass[0])) {
			...
			$conn = connect();
			$qb = $conn->query("SELECT * FROM `companies` WHERE `bid`=".$bid[0]);
			$conn->close();
			while ($row = mysqli_fetch_array($qb, MYSQLI_ASSOC)) {
				$badmin[] = $row['admin'];
			}
